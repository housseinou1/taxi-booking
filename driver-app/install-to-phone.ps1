...
$wavSrc = Join-Path $assetsSrc "notification.wav"
$wavRoot = Join-Path (Split-Path $assetsSrc -Parent) "notification.wav"
if (Test-Path $wavSrc) {
  Copy-Item $wavSrc $wavRoot -Force
  Write-Host "Copied notification.wav to Android assets root"
}
...
if (-not (Test-Path $apkBase)) {
  Write-Host "No local base APK - pulling installed Yala Driver from phone..."
  $pathLine = & $adb -s $serial shell pm path $package 2>&1 | Select-String "package:"
  if (-not $pathLine) {
    Write-Error "Yala Driver not on phone and no debug APK on PC. Open driver-app\android in Android Studio, Build APK, then re-run this script."
  }
...
Write-Host "Patching APK with latest driver UI..."
$patchFile = Join-Path $env:TEMP "yala-driver-patch.py"
@'
...
wav_src = os.path.join(assets_src, "notification.wav")
if os.path.isfile(wav_src):
    shutil.copy2(wav_src, os.path.join(extracted, "assets", "notification.wav"))
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Host "Done. Yala Driver installed on your phone."
} else {
  exit $LASTEXITCODE
}
