...
if (-not (Test-Path $apkBase)) {
  Write-Host "No local base APK - pulling installed Yala Admin from phone..."
  $pathLine = & $adb -s $serial shell pm path $package 2>&1 | Select-String "package:"
  if (-not $pathLine) {
    Write-Error "Yala Admin not on phone and no debug APK on PC. Open admin-app\android in Android Studio, Build APK, then re-run this script."
  }
...
Write-Host "Patching APK with latest admin UI..."
$patchFile = Join-Path $env:TEMP "yala-admin-patch.py"
@'
...
if ($LASTEXITCODE -eq 0) {
  ...
  Write-Host "Done. Yala Admin installed on your phone."
} else {
  exit $LASTEXITCODE
}
