"""
Centralized fare resolver for Yala ride pricing.

Resolution order:
  1. Active CityPricing record for the city and ride type.
  2. Active GlobalFareConfig (database-backed global configuration).
...
def resolve_ride_fare(city, ride_type, distance_km):
    """Resolve ride fare following the approved resolution order:
    ...
    # Resolve policy references at ride creation time
...
# ── Snapshot-aware per-ride policy resolvers ──────────────────────────────────
