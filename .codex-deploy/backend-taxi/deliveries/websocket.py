...
def delivery_chat_group(delivery_id):
    return f"delivery_chat_{delivery_id}"


async def send_delivery_chat_event(channel_layer, delivery_id, event_type, payload):
    """Broadcast delivery chat events to delivery_chat_{delivery_id} group."""
    message = {
        "type": event_type,
        "delivery_id": delivery_id,
        **payload,
    }
    group = delivery_chat_group(delivery_id)
    await channel_layer.group_send(
        group,
        {"type": "delivery_chat_event", "message": message},
    )


async def send_delivery_chat_closed(channel_layer, delivery_id, reason):
    await send_delivery_chat_event(
        channel_layer,
        delivery_id,
        "chat_closed",
        {"reason": reason},
