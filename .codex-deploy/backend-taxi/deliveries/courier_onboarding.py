...
def _application_submitted(profile: DriverProfile | None) -> bool:
    ...
    if profile.status == "approved":
        return True
    return bool(
        courier_has_complete_signature(profile)
        and profile.status == "pending_review"
    )
...
def build_courier_onboarding_state(user, request=None) -> dict:
    ...
    requires_resign = courier_requires_resign(profile) or (
        approved and not courier_terms_version_current(profile)
    )
    can_deliver = ready and not requires_resign
    if requires_resign:
        ready = False
    if is_suspended:
    ...
    elif requires_resign:
        message = "Updated terms require your electronic signature before you can deliver."
    elif not signature_complete and not application_submitted:
        message = "Sign the courier agreement electronically before submitting your application."
