...
class RideConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.user or not self.user.is_authenticated:
            logger.warning(
                "WebSocket rejected (unauthenticated): %s", self.channel_name
            )
    ...
    async def disconnect(self, close_code):
        ...
        for delivery_chat_group in list(self.delivery_chat_groups):
            await self.channel_layer.group_discard(
                delivery_chat_group, self.channel_name
            )
        self.delivery_chat_groups.clear()
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        elif msg_type == "join_delivery_chat":
            await self._handle_join_delivery_chat(data)
        elif msg_type == "leave_delivery_chat":
            await self._handle_leave_delivery_chat(data)
        elif msg_type == "delivery_chat_typing":
            await self._handle_delivery_chat_typing(data)
    ...
    async def _handle_join_delivery_chat(self, data):
        """Join delivery chat group with participant authorization."""
        if not self.user or not self.user.is_authenticated:
        ...
        if not delivery_id:
            await self.send(
                text_data=json.dumps(
                    {"error": "join_delivery_chat requires 'delivery_id'"}
                )
            )
            return

        allowed = await self._user_can_access_delivery_chat(delivery_id)
        if not allowed:
            await self.send(text_data=json.dumps({"error": "Not authorized for delivery chat"}))
            return

        chat_group = f"delivery_chat_{delivery_id}"
        await self.channel_layer.group_add(chat_group, self.channel_name)
        self.delivery_chat_groups.add(chat_group)

        await self.send(
            text_data=json.dumps(
                {"type": "joined_delivery_chat", "delivery_id": delivery_id}
            )
        )

    async def _handle_leave_delivery_chat(self, data):
        delivery_id = data.get("delivery_id")
        if not delivery_id:
            await self.send(
                text_data=json.dumps(
                    {"error": "leave_delivery_chat requires 'delivery_id'"}
                )
            )
            return
        ...
        self.delivery_chat_groups.discard(chat_group)

        await self.send(
            text_data=json.dumps(
                {"type": "left_delivery_chat", "delivery_id": delivery_id}
            )
        )

    async def _handle_delivery_chat_typing(self, data):
        if not self.user or not self.user.is_authenticated:
            return
    ...
    async def delivery_chat_event(self, event):
        """Forward delivery chat events (message_sent, message_read, typing, chat_closed)."""
        try:
            await self.send(text_data=json.dumps(event["message"]))
        except Exception as exc:
            logger.error("WebSocket send error (delivery_chat_event): %s", exc)

    async def delivery_chat(self, event):
        """Legacy delivery chat handler used by template chat broadcasts."""
        try:
            await self.send(text_data=json.dumps(event["message"]))
        except Exception as exc:
            logger.error("WebSocket send error (delivery_chat): %s", exc)
