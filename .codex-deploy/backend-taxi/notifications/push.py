...
def send_push_notification(fcm_token, title, body, data=None, android_channel_id="yala_rides"):
    ...
    android_sound = "delivery_request" if android_channel_id == "yala_deliveries" else "default"
    try:
...
def send_push_to_user(user, title, body, data=None, app_type=None, android_channel_id="yala_rides"):
    ...
    if app_type:
        allowed = {app_type, "web"}
        if app_type == "delivery":
            allowed.add("driver")
        tokens = tokens.filter(app_type__in=tuple(allowed))
...
def notify_new_delivery_request(driver_user_id, delivery):
    ...
    return send_push_to_user(
        ...
        f"{delivery.pickup} → {delivery.destination} · {delivery.fare} MRU",
        {
            ...
            "pickup": delivery.pickup or "",
            "destination": delivery.destination or "",
            "fare": str(delivery.fare or ""),
            "distance_km": str(delivery.distance_km or ""),
