...
from .serializers import (
    AuditLogSerializer,
    CustomerDeliveryDefaultsSerializer,
...
def _serialize_courier_admin_item(profile, request):
    ...
    return {
        ...
        "legal_signature": serialize_courier_signature(profile, request),
    }
...
@api_view(["GET", "PUT"])
@permission_classes([IsAuthenticated])
def delivery_instruction_defaults(request):
    defaults, _ = CustomerDeliveryDefaults.objects.get_or_create(user=request.user)
    if request.method == "GET":
        return Response(CustomerDeliveryDefaultsSerializer(defaults).data)

    serializer = CustomerDeliveryDefaultsSerializer(defaults, data=request.data, partial=True)
    serializer.is_valid(raise_exception=True)
    serializer.save()
    return Response(serializer.data)
...
def admin_courier_action(request, driver_id):
    ...
    if action == "approve":
        if not courier_has_complete_signature(profile):
            return Response(
                {
                    "error": "Courier electronic signature is incomplete. Cannot approve.",
                    "legal_signature": serialize_courier_signature(profile, request),
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        results.append(
            {
                ...
                "legal_signature": serialize_merchant_signature(merchant, request),
            }
        )
...
def admin_merchant_action(request, merchant_id):
    ...
    if action == "approve":
        if not merchant_has_complete_signature(merchant):
            return Response(
                {
                    "error": "Merchant electronic signature is incomplete. Cannot approve.",
                    "legal_signature": serialize_merchant_signature(merchant, request),
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
